  ...
  public void generateHistorical(int days) {
    ...
    if (days < 0 || days > 365) {
      throw new IllegalArgumentException(
          "Invalid days " + String.valueOf(days) + " for historical request.");
    }
    ...
    MeterReadingResource meterResource =
        new MeterReadingResource(siteStatsDao, metricDao, capacityDao, feedDao);

    Set<Site> sites = getAllSites();
    int minuteDays = days * 3 * 60;

    List<Site> sortedSites = sites.stream().sorted().collect(Collectors.toList());

    MeterReading[][] readings = new MeterReading[sortedSites.size()][minuteDays];
    ...
    for (Site site : sortedSites) {
      ...
      for (int i = 0; i < minuteDays; i++) {
        MeterReading reading =
            new MeterReading(
                site.getId(), currentTime, currentUsage, currentCapacity, currentTemperature);
