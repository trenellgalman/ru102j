...
public class ConnectionExamples {
  ...
  public static Jedis getJedis(String host, Integer port, int timeout, String password) {
  ...
  public static JedisPool getPool(
      String host, Integer port, int maxConnections, int timeout, String password) {
  ...
  public static JedisSentinelPool getJedisSentinelPool(
    ...
    return new JedisSentinelPool(masterName, sentinels, config, timeout, password);
  }

  /**
   * Connect to a Redis Cluster deployment. Uses a pool connection under the hood and is threadsafe.
   */
  public static JedisCluster getClusterConnection(
      int timeout, int maxAttempts, int maxConnections) {
