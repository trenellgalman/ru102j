...
/**
 * Retain metrics using the Redis Time Series module (see
 * https://github.com/RedisLabsModules/RedisTimeSeries)
 */
public class MetricDaoRedisTSImpl implements MetricDao {
  private static final Integer RETENTION_MS = 60 * 60 * 24 * 14 * 1000;
  ...
  public void insert(MeterReading reading) {
    insertMetric(
        ...
        reading.getDateTime());
    insertMetric(
        reading.getSiteId(), reading.getWhUsed(), MetricUnit.WHUsed, reading.getDateTime());
    insertMetric(
        reading.getSiteId(),
        reading.getTempC(),
  ...
  private void insertMetric(Long siteId, Double value, MetricUnit unit, ZonedDateTime dateTime) {
