...
/**
 * Methods to generate key names for Redis data structures. These key names are used by the
 * RedisDaoImpl classes. This class therefore contains a reference to all possible key names used by
 * this application.
 */
public class RedisSchema {
  ...
  public static String getSiteStatsKey(Long siteId, ZonedDateTime dateTime) {
    return KeyHelper.getKey(
        "sites:stats:" + getYearMonthDay(dateTime) + ":" + String.valueOf(siteId));
  }
  ...
  static String getRateLimiterKey(String name, int minuteBlock, long maxHits) {
    return KeyHelper.getKey(
        "limiter:" + name + ":" + String.valueOf(minuteBlock) + ":" + String.valueOf(maxHits));
  }
  ...
  // Redis type: sorted set
  static String getDayMetricKey(Long siteId, MetricUnit unit, ZonedDateTime dateTime) {
