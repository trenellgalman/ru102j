...
public class SiteGeoResource {
  ...
  public Response getSites(
      ...
      @QueryParam("onlyExcessCapacity") Boolean onlyExcessCapacity) {
    if (lng == null && lat == null) {
      return Response.ok(siteDao.findAll()).header("Access-Control-Allow-Origin", "*").build();
    } else if (lng != null && lat != null) {
      Set<Site> results = doGeoQuery(lng, lat, radius, radiusUnit, onlyExcessCapacity);
      return Response.ok(results).header("Access-Control-Allow-Origin", "*").build();
    }
    return Response.noContent().status(404).header("Access-Control-Allow-Origin", "*").build();
  }

  private Set<Site> doGeoQuery(
      String lng, String lat, Double radius, String radiusUnit, Boolean onlyExcessCapacity) {
    ...
    if (radiusUnit == null
        || !(radiusUnit.equals("M")
            || radiusUnit.equals("KM")
            || radiusUnit.equals("FT")
            || radiusUnit.equals("MI"))) {
