...
public class MetricsResource {
  ...
  @GET
  @Path("/{siteId}")
  public Response getSiteMetrics(
      @PathParam("siteId") Long siteId, @PathParam("count") Integer count) {
    List<Plot> plots = new ArrayList<>();
    if (count == null) {
      count = DEFAULT_METRIC_COUNT;
    }
    // Get kWhGenerated measurements
    List<Measurement> generated =
        metricDao.getRecent(
            siteId, MetricUnit.WHGenerated, ZonedDateTime.now(ZoneOffset.UTC), count);
    plots.add(new Plot("Watt-Hours Generated", generated));

    // Get kWhUsed measurements
    List<Measurement> used =
        metricDao.getRecent(siteId, MetricUnit.WHUsed, ZonedDateTime.now(ZoneOffset.UTC), count);
    plots.add(new Plot("Watt-Hours Used", used));

    return Response.ok(plots).header("Access-Control-Allow-Origin", "*").build();
  }
}
