...
public class LoadCommand extends Command {
  ...
  public void configure(Subparser subparser) {
    subparser
        ...
        .setDefault(false)
        .help(
            "Run the 'flushdb' command on the Redis database to "
    ...
    subparser
        ...
        .required(false)
        .help(
            "The filename containing the JSON to load. If not specified, "
  ...
  public void run(Bootstrap<?> bootstrap, Namespace namespace) throws Exception {
    ...
    System.out.println(
        "Using Redis at " + (String) namespace.get("host") + ":" + namespace.get("port"));
    if (password.length() > 0) {
      jedisPool =
          new JedisPool(
              new JedisPoolConfig(),
              (String) namespace.get("host"),
              namespace.get("port"),
              2000,
              (String) namespace.get("password"));
    } else {
